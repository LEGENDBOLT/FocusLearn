...
// The entire worker code as a string literal to be used in a Blob.
// This avoids cross-origin issues in sandboxed environments.
const workerCode = `
  let timerId;
  let remainingTime;
...
const TimerView: React.FC = () => {
  ...
  const workerUrlRef = useRef<string | null>(null); // To store the object URL for cleanup
  ...
  useEffect(() => {
    try {
      ...
      workerRef.current.onmessage = (e: MessageEvent<{ type: string; time: number }>) => {
        if (e.data.type === 'tick') {
      ...
      // Set initial time in the worker
  ...
  // Effect to update timer when duration changes from UI, but only if timer is not active
  useEffect(() => {
    if (!isActive) {
  ...
  const handleDurationChange = (newDurationInMinutes: number) => {
