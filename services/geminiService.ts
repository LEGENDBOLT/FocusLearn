...
const getApiKey = (): string => {
    // 1. Prova a leggere dal localStorage
...
// FIX: Added currentApiKey to track the key used for initialization
let currentApiKey: string | null = null;
// FIX: Moved chat to module scope so it can be reset when the AI client is re-initialized.
let chat: Chat | null = null;

const getAI = () => {
    const apiKey = getApiKey();
    // FIX: Re-initialize if ai is not created yet, or if the API key has changed.
    if (!ai || currentApiKey !== apiKey) {
        ai = new GoogleGenAI({ apiKey });
        currentApiKey = apiKey;
        chat = null; // Also reset chat instance since it depends on the `ai` instance
    }
    return ai;
}
...
export const analyzeStudySessions = async (sessions: StudySession[]): Promise<AnalysisResult> => {
  const ai = getAI();
